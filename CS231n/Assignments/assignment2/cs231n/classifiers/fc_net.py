...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        N = X.shape[0]
        D = np.prod(X.shape[1:])
        out1, cache1 = affine_relu_forward(np.reshape(X, (N,D)), W1, b1)
        ...
        ############################################################################
        reg_loss = (np.sum(W1**2) + np.sum(W2**2)) * self.reg * 0.5
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            self.params['W' + str(i + 1)] = np.random.randn(layers_dim[i], layers_dim[i + 1]) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers + 1):
            reg_loss += self.reg * 0.5 * np.sum(self.params['W%d' % i] ** 2)
        loss, dneuron_output = softmax_loss(scores, y)
        loss += reg_loss
        #you just divide the loss by N here
        #how could you be so stupid
        for i in range(1,self.num_layers + 1):
            num_layer = self.num_layers + 1 - i
            ...
            grads['W%d' % (num_layer)] = dw
            grads['b%d' % (num_layer)] = db
